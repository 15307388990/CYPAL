package com.cypal.ming.paotui.view;
...
//            canvas.drawCircle(x, y, dp7, linePaint);
            ...
            drawFloatTextBox(canvas, x, y - dp4, value.get(xValue.get(i)));
//            //绘制普通的节点
//            linePaint.setStyle(Paint.Style.FILL);
...
//            canvas.drawCircle(x, y, dp2, linePaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                System.out.println("xInit" + xInit + "maxXinit" + maxXInit);
    ...
    private void scrollAfterActionUp() {
        ...
        animator.setDuration((long) (scrollLength / (maxXInit - minXInit) * 200));//时间最大为1000毫秒，此处使用比例进行换算
    ...
    public void setValue(Map<String, Float> value, List<String> xValue, List<Float> yValue) {
        int widthPix = getWidth();
        System.out.println("width" + widthPix);
        this.value = value;
        this.xValue = xValue;
        this.yValue = yValue;
        minXInit = width - (width - xOri) * 0.1f - interval * (xValue.size() - 1);
        if (xValue.size() == 7) {
            if (widthPix >= 1050) {
                this.xInit = 100;
            } else if (widthPix == 702) {
                this.xInit = 36;
            } else {
                this.xInit = 128;
            }

        } else if (xValue.size() == 11) {
            if (widthPix >= 1050) {
                this.xInit = -413;
            } else if (widthPix == 702) {
                this.xInit = -360;
            ...
            if (widthPix >= 1050) {
                this.xInit = (xValue.size() - 7) * -135;
            } else if (widthPix == 702) {
                this.xInit = (xValue.size() - 7) * -100;
            } else {
                this.xInit = (xValue.size() - 7) * -97;
            }
