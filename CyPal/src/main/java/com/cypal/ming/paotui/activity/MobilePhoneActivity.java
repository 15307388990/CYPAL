 ...
 * @version 验证手机
 */
public class MobilePhoneActivity extends BaseActivity implements TextWatcher, AuthCodeDialog.onConfirmListener {
...
//        map.put("auth_token", partnerBean.getAuth_token());
//        mQueue.add(ParamTools.packParam(Const.bindPhone, this, this, map));
//        loading();
//    }
    ...
    private void validSafePhone() {
        ...
        mQueue.add(ParamTools.packParam(Const.validSafePhone, this, this, map));
        loading();
    }
    ...
    public void onResponse(String response, String url) {
        ...
        try {
            ...
            if (status == 0) {
                ...
                } else if (url.contains(Const.sendPhoneMsg)) {
                    if (authCodeDialog != null && authCodeDialog.isShowing()) {
                        authCodeDialog.dismiss();
                    }
                    countTimer.start();// 开启定时器
                    tv_code.setVisibility(View.VISIBLE);
                } else if (url.contains(Const.validSafePhone)) {
...
//                }
                ...
                if (url.contains(Const.sendSafeMsg)) {
                    if (authCodeDialog == null) {
                        authCodeDialog = new AuthCodeDialog(getApplicationContext(), MobilePhoneActivity.this,null);
                ...
                if (url.contains(Const.sendPhoneMsg) || url.contains(Const.sendPhoneMsg)) {
                    if (authCodeDialog != null) {
