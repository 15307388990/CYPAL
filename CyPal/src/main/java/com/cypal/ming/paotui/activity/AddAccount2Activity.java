 ...
 *
 */
package com.cypal.ming.paotui.activity;
...
import com.cypal.ming.paotui.base.BaseActivity;
import com.cypal.ming.paotui.config.Const;
import com.cypal.ming.paotui.dialog.AuthCodeDialog;
import com.cypal.ming.paotui.utils.MyCountTimer;
 ...
 * @version 添加账户
 */
public class AddAccount2Activity extends BaseActivity implements TextWatcher, AuthCodeDialog.onConfirmListener {
...
//        pay_pass_token = getIntent().getStringExtra("pay_pass_token");
    ...
    private void getBankName(String card_no) {
        ...
        mQueue.add(ParamTools.packParam(Const.getBankName, this, this, map));
    }
    ...
    private void sendSafeMsg() {
        ...
        Map<String, String> map = new HashMap<>();
        map.put("auth_token", partnerBean.getAuth_token());
        mQueue.add(ParamTools.packParam(Const.sendSafeMsg, this, this, map));
        loading();
    }
    ...
    public void clickMethod(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_banl_cb:   //bank的操作
                ...
                ll_alipay.setVisibility(View.GONE);
                ll_banl.setVisibility(View.VISIBLE);
                et_banl_number.setFocusable(true);
                et_banl_number.requestFocus();
                account_type = 2;
                break;
            case R.id.ll_alipay_cb: //支付宝
                ...
                ll_banl.setVisibility(View.GONE);
                account_type = 1;
                et_alipay_number.setFocusable(true);
                et_alipay_number.requestFocus();
                break;
            case R.id.tv_banl_line: //输入开户行名称
                if (tv_banl_adds.getText().toString().length() > 1) {
                    Intent intent = new Intent(AddAccount2Activity.this, SelectCity.class);
        ...
        try {
            ...
            if (status == 0) {
                ...
                if (url.contains(Const.sendSafeMsg)) {
                    ...
                    authCodeDialog = new AuthCodeDialog(getApplicationContext(), AddAccount2Activity.this, null);
    ...
    private boolean initBtn() {
        if (et_code.getText().length() > 0 && et_alipay_name.getText().length() > 0
            ...
            btn_next.setBackgroundResource(R.drawable.bt);
            ...
            btn_next.setBackgroundResource(R.drawable.bt_no);
